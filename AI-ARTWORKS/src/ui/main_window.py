...
class MainWindow(QMainWindow):
    """Al-artworks main application window"""
    
    operation_complete = Signal()
    
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Al-artworks - AI Creative Studio")
    ...
    def _on_init_complete(self, _):
        ...
        logger.info("Al-artworks initialization complete")
        self.status_bar.showMessage("Ready")
    ...
    def _run_image_edit(self, instruction):
        async def edit_and_update():
            from .. import AlArtworks
            al_artworks = AlArtworks()
            await al_artworks.initialize()
            result = await al_artworks.edit_image(self.current_image, instruction)
            if result and result.get("output_image"):
                self._append_chat("Al-artworks: Edit complete.", user=False)
                ...
                self._append_chat("Al-artworks: Edit failed or not recognized.", user=False)
    ...
    def _on_sketch_made(self, qimage):
        ...
        self._append_chat(
            "Al-artworks: Sketch received. Processing...",
    ...
    def handle_gesture_input(self, gesture_data):
        self._append_chat(
            f"Al-artworks: Gesture input received. Processing...",
            user=False, thinking=True
        )
    ...
    def _handle_user_intent(self, text, input_mode="text"):
        ...
        if intent in ["edit_image", "restore", "style_transfer", "sketch", "gesture"]:
            if self.current_image or input_mode in ("sketch", "gesture"):
                ...
                self._append_chat("Al-artworks: Processing your request...", user=False, thinking=True)
                self.start_operation(lambda: self._process_orchestrator_task(task))
            else:
                self._append_chat("Al-artworks: Please load an image first.", user=False)
        elif intent == "chat":
            self._append_chat("Al-artworks: ...", user=False, thinking=True)
            def llm_response():
                response = self.llm_manager.chat(text)
                self._replace_last_chat(f"Al-artworks: {response}", user=False)
            threading.Thread(target=llm_response, daemon=True).start()
        elif intent == "explain":
            self._append_chat("Al-artworks: Explaining...", user=False, thinking=True)
            def explain_response():
                response = self.llm_manager.explain(text)
                self._replace_last_chat(f"Al-artworks: {response}", user=False)
            threading.Thread(target=explain_response, daemon=True).start()
        else:
            self._append_chat("Al-artworks: Sorry, I didn't understand. Please try a different command.", user=False)
    ...
    def _process_orchestrator_task(self, task):
        ...
        if result and isinstance(result, dict):
            ...
            self._append_chat(
                "Al-artworks: No result or error from orchestrator.", user=False
            )
        return result
    ...
    def run_tree_of_thought(self):
        ...
        if not agent_names:
            self._append_chat("Al-artworks: No agents registered for ToT.", user=False)
            return
        self._append_chat("Al-artworks: Running Tree-of-Thought reasoning...", user=False, thinking=True)
        def do_tot():
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            result = loop.run_until_complete(
            ...
            return result
        def on_done(result):
            self._replace_last_chat("Al-artworks: Tree-of-Thought complete.", user=False)
